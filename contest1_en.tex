...
\begin{itemize}
\item If $a_i=1$, let $p_i=\bigl|\{j<i\mid a_j=1\}\bigr|$ be the number of
$1$'s standing before it. If $p_i\ge 2$, position $i$ issues the instruction
``put a $1$ into position $i+p_i-1$''.
\item If $a_i=0$, let $s_i=\bigl|\{j>i\mid a_j=0\}\bigr|$ be the number of
$0$'s standing after it. If $s_i\ge 2$, position $i$ issues the instruction
...
\paragraph{Question.}
Prove that $T(K)$ always has a \emph{fixed point}: there exists an index
