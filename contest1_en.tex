...
\paragraph*{Notes}
\begin{itemize}
\item[(i)] Every polynomial equation of degree $\le4$ is solvable by
radicals (the quadratic formula, Cardano, and Ferrari).

\item[(ii)] (Abel--Ruffini) There is no general formula in radicals for
polynomial equations of degree $\ge5$; for example, no root of
$x^5-4x+2$ can be expressed by radicals.

\item[(iii)] (casus irreducibilis) If an irreducible cubic over
$\mathbb{Q}$ has three real roots, then none of its roots can be expressed
by real radicals.
\end{itemize}

\subsection*{Problem 4 \quad\normalfont\normalsize (20 points)}
