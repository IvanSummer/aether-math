...
\begin{itemize}
\item 若 $a_i=1$，令 $p_i=\bigl|\{j<i\mid a_j=1\}\bigr|$ 为它前面 $1$ 的个数。
若 $p_i\ge 2$，则位置 $i$ 发出指令：``把第 $i+p_i-1$ 位改成 $1$''。
\item 若 $a_i=0$，令 $s_i=\bigl|\{j>i\mid a_j=0\}\bigr|$ 为它后面 $0$ 的个数。
若 $s_i\ge 2$，则位置 $i$ 发出指令：``把第 $i-s_i+1$ 位改成 $0$''。
\end{itemize}
