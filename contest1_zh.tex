...
欢迎参加第一届灵息杯数学公开赛！
比赛时间为 2026 年 9 月 1 日至 12 月 31 日。
...
对子域 $K\subseteq\mathbb{C}$ 与集合 $S\subseteq\mathbb{C}$，$K(S)$ 表示
包含 $K$ 与 $S$ 的最小子域；``在 $K$ 上不可约''只对非常数多项式而言，即它不是
$K[x]$ 中两个非常数多项式的乘积。
...
$x^5-4x+2$ 的任一根都不能用根式表示。更一般地，对每个 $n\ge5$，
$x^n-x-1$ 在 $\mathbb{Q}$ 上不可约，并且在由 $\mathbb{Q}$ 反复添加根式所得的
任意域上仍不可约；特别地，它的根都不能用根式表示。

(iii)（不可约情形 / casus irreducibilis）若 $\mathbb{Q}$ 上的不可约三次多项式
有三个实根，则它的任一根都不能用实根式表示，无论嵌套多深。

(iv) 若 $g\in K_j[x]$ 的次数 $\le 4$，则由 (i)，$g$ 的每个根都属于某个
$K_{j'}$（$j'\ge j$）。因此，若 $f\in\mathbb{Z}[x]$ 在某个 $K_j$ 上有次数
$\le4$ 的因式，则 $f$ 有一个可用根式表示的根。
...
按下列规则由 $K$ 得到同样长度的新序列 $T(K)=(b_1,\ldots ,b_n)$。

\paragraph{第一步：发出指令。}
每个位置 $i$ \emph{至多}发出一条指令。
\begin{itemize}
\item 若 $a_i=1$，令 $p_i=\#\{j<i\mid a_j=1\}$ 为它前面 $1$ 的个数。若
$p_i\ge 2$，则位置 $i$ 发出指令：``把第 $i+p_i-1$ 位改成 $1$''。
\item 若 $a_i=0$，令 $s_i=\#\{j>i\mid a_j=0\}$ 为它后面 $0$ 的个数。若
$s_i\ge 2$，则位置 $i$ 发出指令：``把第 $i-s_i+1$ 位改成 $0$''。
\end{itemize}
其余情形都不发出指令：数出的个数为 $0$ 或 $1$ 时不发（从自己数起，第一个就是
自己），所指的位置不在 $\{1,\ldots ,n\}$ 内时也不发（不循环、不溢出）。

\paragraph{第二步：同时执行。}
所有指令都由原序列 $K$ 算出，然后同时执行。若位置 $t$ 只收到``改成 $1$''的
指令，则 $b_t=1$；只收到``改成 $0$''的指令，则 $b_t=0$；同时收到两种指令，
或者没有收到任何指令，则 $b_t=a_t$。

\paragraph{问题。}
证明：$T(K)$ 必有\emph{不动点}，即存在 $t\in\{1,\ldots ,n\}$ 使得 $b_t=a_t$。
